...
function PlaceCardItem({activity}: Props) {
  ...
  return (
    <div >
    <Image src={photoUrl?photoUrl:'/placeholder.png'} alt={activity.place_name} width={400} height={200}
    className='w-full h-auto object-cover rounded-xl shadow mb-2'/>
    <h2 className='font-semibold text-lg'>{activity?.place_name}</h2>
    <p className='text-gray-500 line-clamp-2'>{activity?.place_details}</p>
    <h2 className='flex gap-2 text-green-500'><Ticket />{activity?.ticket_pricing}</h2>
    <p className='flex text-blue-500 gap-2'><Clock />{activity?.time_travel_each_location}</p>
